...
unsigned char logroot[200] = "D:\\SYSTEMLOG\\FFD_COAP_SYSTEM_LOG.TXT";
FILE* logfile = NULL;
unsigned char dumpingroot[200] = "D:\\BIOLOG\\";
unsigned char configroot[200] = "D:\\FFDCONFIG\\FFDCONFIG.INI";
unsigned char* DUMPINGROOT_CUT;
...
typedef struct _DICTIONARY {
    coap_string_t id;
    coap_string_t topic;
    coap_string_t filename;
    coap_string_t fileroot;
    unsigned int toggle;
    unsigned int maxnum;
    unsigned int difnum;
    unsigned int pernum;
...
void DICTIONARY_INIT(dictionary* head){
    head->count = 0;
    head->head = NULL;
}

unsigned int DICTIONARY_ADD(dictionary* head, unsigned char* id, unsigned char* topic, unsigned char* filename, unsigned int maxnum) {
    ...
    while (1) {
        ...
        if (head->count == 0/*temp == NULL*/) {
            temp = (_dictionary*)coap_malloc(sizeof(_dictionary));
            //id
            temp->id.length = strlen(id);
            temp->id.s = (unsigned char*)coap_malloc(temp->id.length + 1);
            memcpy(temp->id.s, id, temp->id.length + 1);
            //topic
            temp->topic.length = strlen(topic);
            temp->topic.s = (unsigned char*)coap_malloc(temp->topic.length + 1);
            memcpy(temp->topic.s, topic, temp->topic.length + 1);
            //filename
            temp->filename.length = strlen(filename);
            temp->filename.s = (unsigned char*)coap_malloc(temp->filename.length + 1);
            ...
            temp->fileroot.s = (unsigned char*)coap_malloc(temp->fileroot.length + 1);
        ...
        // 헤더(현재 노드)가 마지막 일때
            ...
            temp->link = (_dictionary*)coap_malloc(sizeof(_dictionary));
            //id
            temp->link->id.length = strlen(id);
            temp->link->id.s = (unsigned char*)coap_malloc(temp->link->id.length + 1);
            memcpy(temp->link->id.s, id, temp->link->id.length + 1);
            //topic
            temp->link->topic.length = strlen(topic);
            temp->link->topic.s = (unsigned char*)coap_malloc(temp->link->topic.length + 1);
            memcpy(temp->link->topic.s, topic, temp->link->topic.length + 1);
            //filename
            temp->link->filename.length = strlen(filename);
            temp->link->filename.s = (unsigned char*)coap_malloc(temp->link->filename.length + 1);
            ...
            temp->link->fileroot.s = (unsigned char*)coap_malloc(temp->link->fileroot.length + 1);
...
_dictionary* DICTIONARY_SEARCH(dictionary* head, unsigned char* id) {
    _dictionary* temp = head->head;
    while (1) {
...
unsigned int DICTIONARY_MODIFY(dictionary* head, unsigned char* id, unsigned char* topic, unsigned char* filename, unsigned int maxnum) {
    ...
    if (temp == NULL) { return 0; }
        ...
        coap_free(temp->id.s);
        coap_free(temp->topic.s);
        coap_free(temp->filename.s);
        coap_free(temp->fileroot.s);
        fclose(temp->file);
...
unsigned int DICTIONARY_DELETE(dictionary* head, unsigned char* id) {
    ...
    if (pre == NULL) {
        coap_free(ptr->id.s);
        coap_free(ptr->topic.s);
        coap_free(ptr->filename.s);
        coap_free(ptr->fileroot.s);
        fclose(ptr->file);
        coap_free(ptr);
        head->head = NULL;
    }
...
void DICTIONARY_FREE(dictionary* head) {
    _dictionary* ptr = head->head;
...
static unsigned int
append_to_output2(const uint8_t* data, size_t len, FILE* file) {
    size_t written;
    do {
        written = fwrite(data, 1, len, file);
        len -= written;
...
static void
hnd_get_command(coap_resource_t* resource,
    coap_session_t* session,
    const coap_pdu_t* request,
    const coap_string_t* query,
    coap_pdu_t* response
    ...
    if (query) {
        ...
        if (strcmp(cut, "READY") == 0) {
            ...
            len = sprintf((unsigned char*)buf, "ERR:W");
            coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
            coap_add_data_large_response(resource, session, request, response,
                query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
                len,
        ...
        len = sprintf((unsigned char*)buf, "ERR:N");
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
        coap_add_data_large_response(resource, session, request, response,
            query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
            len,
...
static void
hnd_put_file(coap_resource_t* resource,
    coap_session_t* session,
    const coap_pdu_t* request,
    const coap_string_t* query,
    coap_pdu_t* response
    ...
    if (rfdcli == NULL) {
    ...
    else if (coap_get_data_large(request, &size, &data, &offset, &total)) {
        append_to_output2(data, size, rfdcli->file);
        ...
        len = sprintf((unsigned char*)buf, "GOOD");
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
        coap_add_data_large_response(resource, session, request, response,
            query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
            len,
        ...
        len = sprintf((unsigned char*)buf, "ERR:R");
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CHANGED);
        coap_add_data_large_response(resource, session, request, response,
            query, COAP_MEDIATYPE_TEXT_PLAIN, 1, 0,
            len,
...
init_resources(coap_context_t *ctx) {
  ...
  r = coap_resource_init(coap_make_str_const("command"), resource_flags);
  coap_register_request_handler(r, COAP_REQUEST_GET, hnd_get_command);
  coap_resource_set_get_observable(r, 1);

  coap_add_attr(r, coap_make_str_const("ct"), coap_make_str_const("0"), 0);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"command\""), 0);
  coap_add_resource(ctx, r);

  r = coap_resource_init(coap_make_str_const("file"), resource_flags);
  coap_register_request_handler(r, COAP_REQUEST_PUT, hnd_put_file);
  coap_resource_set_get_observable(r, 1);

  coap_add_attr(r, coap_make_str_const("ct"), coap_make_str_const("0"), 0);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"file\""), 0);
  coap_add_resource(ctx, r);
...
main(int argc, char **argv) {
  ...
  while ((opt = getopt(argc, argv, "a:D:c:d:eg:G:h:i:j:J:k:l:mnp:rs:u:v:A:C:L:M:NP:R:S:X:")) != -1) {
    switch (opt) {
    case 'a':
